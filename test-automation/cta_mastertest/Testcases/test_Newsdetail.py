...
class Test_Newsdetail(commonfunctions):

    def test_newsdetailcta(self,setup):
        self.logger = Logs.loggen('Test Newsdetail')  # logger of current test case
        self.logger.info("******* Starting CTA TEST NEWSDETAIL**********")
        ...
        df.to_csv('newsdetail.csv', index=False)
        ...
        time.sleep(2)
            ...
            # Create a DataFrame with the results
        data = {
                    'CTA' : [ 'News Subsribe Now', 'News Subsribe Now', 'News Subsribe Now', 'News Subsribe Now','News Subsribe Now', 'News Subsribe Now'],
                    'Platforms' : ['Entered Lead','CLD','LMS', 'SSO', 'Payload', 'Response'],
                    'Lead ID' : [ None, lead_id, lms_lead_id, sso_lead_ID, None, response_message],
                    'Name': [entered_name, lead_name, lms_name, sso_name, sync_name, None],
                    'Email': [entered_email, lead_email, lms_email, sso_email, sync_email, None],
        ...
        df = pd.DataFrame(data)
        # Save DataFrame to CSV
        df.to_csv('newsdetail.csv', mode='a', header=False, index=False)
        ...
        try:
            ...
            self.driver.get(f"https://www.collegedekho.com/news/{news_url}-{URIID}/")
            time.sleep(2)
            second_cta_top = self.driver.find_element(By.XPATH,"/html/body/div[2]/div[1]/div/div[1]/div[1]/div[1]/div[3]/div[2]/button[2]")
            time.sleep(2)
            ...
            time.sleep(2)
            new_url1 = self.driver.current_url
            self.logger.info("New URL : " + new_url1)
            if new_url1 == current_url:
                time.sleep(2)
                current_url = self.driver.current_url
                self.logger.info("Current_Url : " + current_url)
                entered_name, entered_email, entered_phone_no, entered_stream, entered_state, entered_city = self.happyflow()
        ...
        try:
            ...
            self.driver.get(f"https://www.collegedekho.com/news/{news_url}-{URIID}/")
            time.sleep(2)
            single_cta_bottom = self.driver.find_element(By.XPATH,"/html/body/div[3]/div[2]/div/button")
            time.sleep(2)
