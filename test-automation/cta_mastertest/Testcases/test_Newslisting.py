...
class Test_Newslisting(commonfunctions):
        
    
    baseUrl = ReadConfig.NewslistingURL()

    def test_newslistingcta(self, setup):
        self.logger = Logs.loggen('Test Newslisting')  # logger of current test case
        self.logger.info("******* Starting CTA TEST NEWSLISTING**********")
        ...
        df.to_csv('newslisting.csv', index=False)
