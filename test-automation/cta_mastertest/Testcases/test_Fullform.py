...
class Test_Fullform(commonfunctions):
    
    baseUrl = ReadConfig.FullformURL()

    def test_fullformcta(self, setup):
        self.logger = Logs.loggen('Test Fullform')  # logger of current test case
        self.logger.info("******* Starting CTA TEST FULLFORM**********")
        ...
        time.sleep(2)
        register_now = self.driver.find_element(By.XPATH,"//button[@id='btn-cta-125']")
        time.sleep(2)
        register_now_text = register_now.text
        self.logger.info( "CTA Text : " + register_now_text)
        time.sleep(2)
        register_now.click()
        time.sleep(2)
        # self.name_negative()
        ...
        data = {
                'CTA' : [ register_now_text , register_now_text , register_now_text, register_now_text, register_now_text, register_now_text],
        ...
        df.to_csv('fullform.csv', mode='a', header=False, index=False)
