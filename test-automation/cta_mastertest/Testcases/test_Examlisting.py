...
class Test_Examlisting(commonfunctions):
        
    
    baseUrl = ReadConfig.ExamlistingURL()

    def test_examlistingcta(self, setup):
        self.logger = Logs.loggen('Test Examlisting')  # logger of current test case
        self.logger.info("******* Starting CTA TEST EXAMLISTING**********")
        ...
        time.sleep(2)
        register_now = self.driver.find_element(By.XPATH,"(//span[@id='btn-cta-7'])[1]")
        time.sleep(2)
        ...
        df.to_csv('examlisting.csv', index=False)
