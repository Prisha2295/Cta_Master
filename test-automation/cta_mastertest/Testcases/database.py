...
def board_url_connect(logger):
    ...
    query = ("""select slug
                from news_news
                where language_id = 1 && news_type = 7
                order by id desc
                limit 1""")
    
    cursor = conn.cursor()
    cursor.execute(query)
    Boardslug = cursor.fetchone()
    BoardURL = Boardslug[0]
...
def article_url_connect(logger):
    # Create a MySQL database connection
    conn = mysql.connector.connect(host=cld_host_live, database=cld_databasename_live, user=cld_username_live, password=cld_pass_live)
    # conn = mysql.connector.connect(host='95.217.156.247',database = 'collegedekho',user = 'cld_ro', password = 'w4snE59A$dr@k48q')
    ...
    query = ("""select slug
                from news_news
                where language_id = 1 && news_type = 2
                order by id desc
                limit 1""")
    
    cursor = conn.cursor()
    cursor.execute(query)
    Articleslug = cursor.fetchone()
    ArticleURL = Articleslug[0]
...
def SSO_lead(logger, phone_no):
    # Create a MySQL database connection
    # conn = mysql.connector.connect(host=sso_host_staging, database=sso_databasename_staging, user='sso_username_staging, password=sso_pass_staging)
